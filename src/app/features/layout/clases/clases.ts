...
export class Clases {
  ...
  showCalendar = false;
  selectedDate: string = '';
  classesByDay: { [key: string]: ClassSession[] } = {};
  constructor(
  ...
  loadClases() {
    ...
    this.classService.getByDateRange(startStr, endStr).subscribe({
      next: (data) => {
  ...
  groupClassesByDay() {
    ...
    this.weekDays.forEach(day => {
      ...
      this.classesByDay[dateKey] = [];
    });

    this.filteredClases.forEach(clase => {

      const dateKey = clase.startDate;
      if (this.classesByDay[dateKey]) {
        this.classesByDay[dateKey].push(clase);
      }
    });


    Object.keys(this.classesByDay).forEach(key => {
      this.classesByDay[key].sort((a, b) => a.startTime.localeCompare(b.startTime));
    });
  }
  ...
  trackByClaseId(index: number, item: ClassSession): number {
    return item.id!;
  }
  ...
  forkJoin({
    ...
    next: (results) => {
      ...
      this.disciplines = results.disciplines;
  ...
  openClients(c: ClassSession) {
    ...
    if (!c.disciplineId) {
      console.error("Esta clase no tiene disciplina asignada");
      return;
    }


    this.adminService.getClientsByDiscipline(c.disciplineId).subscribe({
      next: (data) => {
      ...
      error: (err) => console.error('Error cargando clientes por disciplina', err)
    });
  }
