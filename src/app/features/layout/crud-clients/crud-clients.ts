...
export class CrudClients implements OnInit {
  ...
  searchTimeout: any;
  isHistoryClassesModalOpen = false;
  selectedHistoryClasses: any[] = [];
  ...
  openPlanClasses(planHistoryItem: any) {
    if (!this.editingClient?.id) return;

    this.selectedPlanName = planHistoryItem.planName;
    this.isLoadingHistory = true;
    this.isHistoryClassesModalOpen = true;
    const start = planHistoryItem.startDate;
    const end = planHistoryItem.endDate;

    this.clientService.getClientClassesHistoryRange(this.editingClient.id, start, end)
      .subscribe({
        next: (data) => {
          this.selectedHistoryClasses = data;
          console.log(data);
        ...
        error: (err) => {
          console.error(err);
          this.isLoadingHistory = false;
          alert('Error cargando el historial de clases');
        }
      });
  }

  closeHistoryClassesModal() {
    this.isHistoryClassesModalOpen = false;
    this.selectedHistoryClasses = [];
  }

  updateAttendance(clase: any, newStatus: string) {
    if (!this.editingClient?.id) return;

    const classId = clase.id;
    const clientId = this.editingClient.id;

    this.attendanceService.markAttendance(classId, clientId, newStatus).subscribe({
      next: () => {
        console.log(`Asistencia actualizada a ${newStatus}`);
  ...
  assignPlan() {
    ...
    // Calculamos visualmente cuánto pagará para mostrarlo en la alerta
    const selectedPlan = this.availablePlans.find(p => p.id === this.selectedPlanId);
    const planPrice = selectedPlan ? selectedPlan.price : 0;
    const finalPrice = Math.max(0, planPrice - this.discountAmount);

    if (confirm(`¿Asignar plan por S/${finalPrice} (Desc: S/${this.discountAmount}) vía ${this.selectedPaymentMethod}?`)) {
